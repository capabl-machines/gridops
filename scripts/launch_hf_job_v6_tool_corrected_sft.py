"""Launch GridOps v6 tool-corrected SFT as a Hugging Face Job.

The job clones the current branch, builds tool-corrected reason-action traces,
validates them, continues SFT from the preserved v5.1 adapter, uploads a new
adapter subfolder, and optionally uploads a holdout eval report.
"""
...
DEFAULT_RUN_LABEL = "sft_qwen25_3b_gridops_v6_tool_corrected"
DEFAULT_INIT_ADAPTER = "77ethers/gridops-models/sft_qwen25_3b_gridops_v51_crisis_repair"
...
def build_job_command(args: argparse.Namespace) -> str:
    exports = {
        ...
        "GRIDOPS_UPLOAD": "1",
        "GRIDOPS_TOOL_CORRECTED_TASKS": args.tasks,
        "GRIDOPS_TOOL_CORRECTED_SEEDS": args.seeds,
        "GRIDOPS_TOOL_CORRECTED_STRIDE": str(args.stride),
        "GRIDOPS_TOOL_CORRECTED_POLICY": args.candidate_policy,
...
api.upload_file(
    ...
    repo_id=repo,
    repo_type="model",
    commit_message=f"Upload GridOps v6 holdout eval {run_label}",
)
PY
fi
'''
    return "\n".join(
        [
            ...
            *_export_lines(exports),
            "python scripts/build_gridops_tool_corrected_sft.py "
            '--tasks "$GRIDOPS_TOOL_CORRECTED_TASKS" '
            '--seeds "$GRIDOPS_TOOL_CORRECTED_SEEDS" '
            '--stride "$GRIDOPS_TOOL_CORRECTED_STRIDE" '
            '--candidate-policy "$GRIDOPS_TOOL_CORRECTED_POLICY" '
            '--output "$GRIDOPS_TRACE_PATH" '
            '--summary-output "evals/gridops_tool_corrected_sft_v6_summary.json" '
            "--shuffle",
...
def main() -> None:
    ...
    parser.add_argument("--trace-path", default="sft_traces/gridops_tool_corrected_sft_v6.jsonl")
    ...
    parser.add_argument("--seeds", default="7301,7302,7303,7304,7305,7306")
    ...
    job = run_job(
        ...
        labels={"project": "gridops", "stage": "v6-tool-corrected-sft"},
        token=token,
    )
